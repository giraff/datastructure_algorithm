...
같은 의미로 진수는 4씩 끊으면 0000 ~ 1111 (0~15)까지이므로 4개씩 자릿수는 16진수 한 잘다.
...
```jsx
(1은 소수 아님)
소수 배열 (n+1)개 true로 초기화
2부터 구하고자 하는 구간의 모든 수를 나열
2는 소수이므로 오른쪽에 2를 쓴다
자기자신을 제외한 2의 배수는 모두 제거
남아있는 수 가운데 3은 소수이므로 오른쪽에 쓴다
자기자신은 제외한 나머지 3의 배수를 모두 제거
남아있는 수 가운데 5는 소수이므로 오른쪽에 쓴다
자기 자신을 제외한 5의 배수를 모두 지운다.

...
위의 과정을 반복하고 남는 수가 소수이다.
if) 1의 20까지 범위를 구한 다면, 11^2 > 120 이므로 
11의 배수까지만 지워도 충분하다.
```

...
export default class LinkedList {
  ...
  reverse() {
    ...
    while (cur_node) {
      // next :: 말그대로 reverse를 진행하는 순서대로 다음 노드를 가리키는 변수. loop을 끝내기 위한 임시 방편의
      next_node = cur_node.next;
      // 방향을 바꾸는 문장. cur_node가 prev를 가리키도록 한다. 첫 노드는 당연히 head의 next로 null을 저장하게 되고
      // 다음부터는 현재 노드가 그 이전 노드를 가리키게 된다 (화살표 방향이 바뀌는 데에 결정적인 역할을 하는 코드)
      cur_node.next = prev_node;

      // 다음으로 진행하기 전 이전 노드 업데이트 코드. 다음 노드 입장에서 현재 노드는 이전 노드이므로 이전 노드로 현재 노드를 설정해준다.
      prev_node = cur_node;
      // cur_node = cur_node.next와 같은 의미 다음으로 진행한다.
      cur_node = next_node;
    }
